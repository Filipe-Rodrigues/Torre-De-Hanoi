...
void jogar (pilha** torres, pilha** auxiliar, int quantidade) {
	...
	if (movimentos == (int) (pow (2, quantidade)) - 1) {
		if (quantidade < 10) {
			...
			imprimeEspacos (7);
			cout << "    Se quer se mostrar, por que não tenta fazer essa porra com " << quantidade + 1 << " peças?" << endl << endl;
		} else {
			...
			imprimeEspacos (7);
			cout << "    Não, sério, vai te fuder, agora você mostrou que é de fato o \"PICA DAS GALÁXIAS\"..." << endl << endl;
			imprimeEspacos (7);
			cout << "    ... em torres de hanoi." << endl << endl << endl;
		}
	} else {
		cout << "    Não seja ridículo. Faz essa merda direito." << endl;
		imprimeEspacos (7);
		cout << "    Com esse tanto de movimento dava pra resolver até duas torres de hanoi." << endl << endl;
	}
...
void jogarAutomaticamente (pilha** torres, pilha** auxiliar, int quantidade) {
	...
	imprimirJogo (true, torres, auxiliar, true, movimentos);
	cout << "    Tá vendo como é que faz, bonachão? Tenta ser menos inútil e joga você mesmo." << endl << endl;
	cout << "    Pressione [ENTER] para continuar";
	string lixo;
	cin.ignore ();
	getline (cin, lixo);
